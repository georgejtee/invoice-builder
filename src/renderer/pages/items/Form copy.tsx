...
export const Form: FC<Props> = ({ handleChange = () => {}, item }) => {
  ...
  useCurrenciesRetrieve({
    onDone: (data: Response<Currency[]>) => {
      if (!data.success) {
        if (data.message) {
          const message = i18n.exists(data.message) ? t(data.message) : data.message;
          dispatch(addToast({ message: message, severity: 'error' }));
      ...
      setCurrencies((data.data ?? []).filter(c => !c.isArchived));
    }
  });
    ...
    <Grid container spacing={2}>
      ...
      <Grid size={{ xs: 12, md: 6 }}>
        <Autocomplete
          ...
          value={currencies.find(opt => opt.id === form.currencyId) ?? null}
          getOptionLabel={option => `${option.code} (${option.symbol})`}
          onChange={(_e, newValue) => {
            update('currencyId', newValue?.id);
            update('currencyCode', newValue?.code);
          }}
